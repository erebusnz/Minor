...
def build_mutation_prompt(file_contents: str, mutation_count: str) -> str:
    return (
        ...
        "Don't propose mutations no host test could *meaningfully* kill: changes "
        "only to nondeterministic-by-design internals -- e.g. a PRNG or hash core "
        "whose sole observable effect is which equally-valid pseudo-random value "
        "appears. These are off-limits even when a fixed seed makes the stream "
        "reproducible: pinning the exact stream is a brittle change-detector, not a "
        "real test. Also skip hardware values the host build stubs out (volatile, "
        "HAL_*, ISR state), and mutations equivalent to the original.\n\n"
...
def build_analysis_prompt(
    ...
    return (
        ...
        f"Results:\n{results_summary}\n\n"
        f"Module source code:\n{file_contents}\n\n"
        "For each SURVIVED mutation, do ONE of:\n"
        "(a) explain why the tests miss it and give a specific, non-brittle Unity "
        "test that kills it; or\n"
        "(b) if killing it would require asserting the exact value or sequence a "
        "PRNG, hash, or similar generator emits, recommend DECLINING and do NOT "
        "write a test. A fixed seed makes such a test deterministic but not "
        "meaningful: any value the generator could emit is equally correct, so the "
        "assertion only detects that the code changed, not that it broke. Treat "
        "'fixed seed => deterministic sequence' as a golden-master change-detector, "
        "NOT a behavioral contract -- do not reframe it as one. The same applies to "
        "any mutation whose only effect is which equally-valid output appears.\n\n"
